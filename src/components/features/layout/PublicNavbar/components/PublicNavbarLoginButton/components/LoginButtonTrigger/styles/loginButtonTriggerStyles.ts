...
export const loginButtonTriggerStyles = tv({
  slots: {
    ...
    // Neon glow layers
    glowLayer1:
      "absolute inset-0 rounded-3xl blur-lg opacity-60 group-hover:opacity-85 transition-opacity duration-300",
    glowLayer2:
      "absolute inset-0 rounded-3xl blur-xl opacity-40 group-hover:opacity-60 transition-opacity duration-300",
  },
  variants: {
    resolvedTheme: {
      dark: {
