...
export const mobileToggleStyles = tv({
  slots: {
    ...
    // Button base styles
    button:
      "relative overflow-hidden rounded-2xl transition-all duration-300 hover:bg-gradient-to-r hover:from-orange-100 hover:to-red-100 dark:hover:from-orange-900/30 dark:hover:to-red-900/30 group-hover:scale-110 group-hover:shadow-lg cursor-pointer",

    // Inner gradient overlay
    innerGradient:
      "absolute inset-0 bg-gradient-to-r from-orange-500/20 to-red-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl",
    ...
    closeIcon:
      "w-6 h-6 text-slate-600 dark:text-slate-400 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors duration-300 group-hover:rotate-90",

    // Menu icon (when menu is closed)
    menuIcon:
      "w-6 h-6 text-slate-600 dark:text-slate-400 group-hover:text-primary dark:group-hover:text-primary-light transition-colors duration-300",
  ...
  variants: {
    mobileOpen: {
      true: {
        indicatorDot: "bg-red-500 scale-125",
      },
      false: {
